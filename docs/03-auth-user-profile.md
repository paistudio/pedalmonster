...
## Auth (MVP scope)
- Email/password registration + login (keep simple for MVP — social login can come later), handled entirely by **Supabase Auth** — the Vue frontend uses its client SDK directly for register/login/logout/session refresh/password reset. There is no backend server involved at all. See `19-supabase-only-backend-plan.md`.
- **Email confirmation is disabled** (`mailer_autoconfirm` on) — registering returns a live session immediately, no "check your email" step. Deliberate MVP tradeoff for frictionless signup; Supabase's free-tier email sending also has a strict rate limit that repeatedly got hit during development, which was the practical push for this decision. Revisit before a real launch if email-verified accounts become a requirement.
- A `profiles` row (this doc's "User" entity minus credentials) is created automatically on sign-up via a Supabase DB trigger
...
- **Login screen**: the `pedalmonster-logo-icon.svg` mark (128×128, 2x the initial size) above a "Welcome Back" title set in **Stoke** (Google Font, loaded alongside Inter/JetBrains Mono in `index.html`) — a one-off, explicitly-requested departure from `design.md`'s `--font-display` (Inter) scoped to just this title, not a system-wide type change — with a one-line tagline beneath ("Bike community platform to buy, sell, and connect with riders near you.") — not the "Pedal Monster" text string, matching the top app bar's use of the mark/wordmark instead of literal text elsewhere in the app.
- **Login screen background**: a looping, muted, autoplaying background video (`frontend/public/videos/login-bg.mp4`, community bike riding on a street) fills the screen behind the form, `object-fit: cover`. A fixed `rgba(10, 10, 10, 0.72)` black overlay (near-black canvas tone, see `design.md`) sits between the video and the form so the video reads as ambient texture, not a competing focal point, and all form text/inputs stay legible. Same treatment should be reused on the register screen if/when it gets a background video — don't invent a different overlay opacity or color per screen.
